...
function getDocsContentPath(slug: string) {
  return path.join(process.cwd(), "/contents/guide/", `${slug}/index.mdx`);
}
...
export async function getAllBlogStaticPaths() {
  try {
    const blogFolder = path.join(process.cwd(), "/contents/posts/");
...
export async function getAllBlogs() {
  ...
  const uncheckedRes = await Promise.all(
    files.map(async (file) => {
      ...
      const filepath = path.join(process.cwd(), `/contents/posts/${file}`);
...
export async function getBlogForSlug(slug: string) {
  const blogFile = path.join(process.cwd(), "/contents/posts/", `${slug}.mdx`);
  try {
